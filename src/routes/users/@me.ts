...
export async function UsersMeGet({
    ...
    if (!t) { res.status(401).json(UNAUTHORIZED); return }
    const user = await prisma.user.findFirst({ where: { id: t.userId } });
    if (!user) { res.status(401).json(UNAUTHORIZED); return; }
...
export async function UsersMePut({
    ...
    if (!body.success) {
        res
            .status(400)
            .json({ error: true, data: body.error.issues.map((e) => e.message) }); return
    }

    const bdy = body.data;
    res.json({ success: true, ...userFormat(await prisma.user.update({ where: { id: t.userId }, data: bdy })) })
}


function userFormat(d: User) {
    return {
        id: d.id,
        username: d.username,
